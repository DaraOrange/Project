...
class LSTMMorphoAnalysis:
    def __init__(self, language: str):
        ...
        self.morph = (
            MorphAnalyzer() if language == "ru" else None
        )  # type: MorphAnalyzer
        self.converter = (
            converters.converter("opencorpora-int", "ud14")
            if self.language == "ru"
            else None
        )
        self.grammeme_vectorizer_input = (
            GrammemeVectorizer()
        )  # type: GrammemeVectorizer
        self.grammeme_vectorizer_output = (
            GrammemeVectorizer()
        )  # type: GrammemeVectorizer
    ...
    def prepare(
        ...
        if (
            self.grammeme_vectorizer_input.is_empty()
            or self.grammeme_vectorizer_output.is_empty()
            or self.word_vocabulary.is_empty()
            or not self.char_set
        ):
    ...
    def save(
        ...
        eval_model_config_path: str,
        eval_model_weights_path: str,
    ):
    ...
    def load_train(
        ...
        with open(model_config_path, "r", encoding="utf-8") as f:
            if config.use_crf:
                ...
                self.train_model = model_from_yaml(
                    f.read(), custom_objects=custom_objects
                )
            else:
        ...
        self.train_model.compile(Adam(clipnorm=5.0), loss=loss, metrics=metrics)

        self.eval_model = Model(
            inputs=self.train_model.inputs, outputs=self.train_model.outputs[0]
        )
    ...
    def load_eval(
        ...
        with open(eval_model_config_path, "r", encoding="utf-8") as f:
            if config.use_crf:
                ...
                custom_objects = {"ReversedLSTM": ReversedLSTM, "CRF": CRF}
                self.eval_model = model_from_yaml(
                    f.read(), custom_objects=custom_objects
                )
            else:
                custom_objects = {"ReversedLSTM": ReversedLSTM}
    ...
    def build(self, config: BuildModelConfig, word_embeddings=None):
        ...
        if config.use_word_embeddings and word_embeddings is not None:
            ...
            words_embedding = Embedding(
                word_vocabulary_size, word_embeddings_dim, name="word_embeddings"
            )(words)
        ...
        if config.use_gram:
            ...
            grammemes_embedding = Dense(config.gram_hidden_size, activation="relu")(
                grammemes_embedding
            )
        ...
        if config.use_crf:
            ...
            out_layer_name = "main_pred"
            outputs.append(
                Dense(num_of_classes, activation="softmax", name=out_layer_name)(layer)
            )
            loss[out_layer_name] = "sparse_categorical_crossentropy"
            metrics[out_layer_name] = "accuracy"
        ...
        if config.use_pos_lm:
            ...
            next_layer_name = "shifted_pred_next"
            prev_layer = Dense(
                num_of_classes, activation="softmax", name=prev_layer_name
            )
            next_layer = Dense(
                num_of_classes, activation="softmax", name=next_layer_name
            )
            outputs.append(
                prev_layer(Dense(config.dense_size, activation="relu")(lstm_backward_1))
            )
            outputs.append(
                next_layer(Dense(config.dense_size, activation="relu")(lstm_forward_1))
            )
            loss[prev_layer_name] = loss[
                next_layer_name
            ] = "sparse_categorical_crossentropy"
        ...
        if config.use_word_lm:
            ...
            outputs.append(
                out_embedding(
                    Dense(word_embeddings.shape[1], activation="relu")(lstm_backward_1)
                )
            )
            outputs.append(
                out_embedding(
                    Dense(word_embeddings.shape[1], activation="relu")(lstm_forward_1)
                )
            )
            loss[out_layer_name] = "sparse_categorical_crossentropy"
            metrics[out_layer_name] = "accuracy"
        ...
        for big_epoch in range(train_config.epochs_num):
            ...
            for epoch, (inputs, target) in enumerate(batch_generator):
                ...
                if epoch != 0 and epoch % train_config.dump_model_freq == 0:
                    self.save(
                        train_config.train_model_config_path,
                        train_config.train_model_weights_path,
                        train_config.eval_model_config_path,
                        train_config.eval_model_weights_path,
                    )
            self.evaluate(
                file_names=file_names,
                val_idx=val_idx,
    ...
    def evaluate(
        ...
        for epoch, (inputs, target) in enumerate(batch_generator):
            predicted_y = self.eval_model.predict(
